...
class TigoWithdrawalSummaryDetail extends StatefulWidget {
  final withdrawal_date;
  const TigoWithdrawalSummaryDetail({Key? key,this.withdrawal_date}) : super(key: key);

  @override
  _TigoWithdrawalSummaryDetailState createState() => _TigoWithdrawalSummaryDetailState(withdrawal_date: this.withdrawal_date);
}

class _TigoWithdrawalSummaryDetailState extends State<TigoWithdrawalSummaryDetail> {
  final withdrawal_date;
  _TigoWithdrawalSummaryDetailState({required this.withdrawal_date});
  late String username = "";
  final storage = GetStorage();
  bool hasToken = false;
  late String uToken = "";
  late List allTigoWithdraws = [];
  bool isLoading = true;
  late var items;
  late List amounts = [];
  ...
  fetchUserTigoWithdraws()async{
    ...
    if(response.statusCode ==200){
      ...
      allTigoWithdraws = json.decode(jsonData);
      amounts.assignAll(allTigoWithdraws);
    ...
    setState(() {
      isLoading = false;
      allTigoWithdraws = allTigoWithdraws;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Tigo withdraws"),
      ),
      body: SafeArea(
          child:
          isLoading ? Column(
