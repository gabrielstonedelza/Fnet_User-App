...
class TigoDepositSummaryDetail extends StatefulWidget {
  final deposit_date;
  const TigoDepositSummaryDetail({Key? key,this.deposit_date}) : super(key: key);

  @override
  _TigoDepositSummaryDetailState createState() => _TigoDepositSummaryDetailState(deposit_date: this.deposit_date);
}

class _TigoDepositSummaryDetailState extends State<TigoDepositSummaryDetail> {
  final deposit_date;
  _TigoDepositSummaryDetailState({required this.deposit_date});
  late String username = "";
  final storage = GetStorage();
  bool hasToken = false;
  late String uToken = "";
  late List allTigoDeposits = [];
  bool isLoading = true;
  late var items;
  late List amounts = [];
  ...
  fetchUserTigoDeposits()async{
    ...
    if(response.statusCode ==200){
      ...
      allTigoDeposits = json.decode(jsonData);
      amounts.assignAll(allTigoDeposits);
    ...
    setState(() {
      isLoading = false;
      allTigoDeposits = allTigoDeposits;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("$username's Tigo deposits"),
      ),
      body: SafeArea(
          child:
          isLoading ? Column(
