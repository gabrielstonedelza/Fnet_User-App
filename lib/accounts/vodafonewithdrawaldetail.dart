...
class VodafoneWithdrawalSummaryDetail extends StatefulWidget {
  final withdrawal_date;
  const VodafoneWithdrawalSummaryDetail({Key? key,this.withdrawal_date}) : super(key: key);

  @override
  _VodafoneWithdrawalSummaryDetailState createState() => _VodafoneWithdrawalSummaryDetailState(withdrawal_date: this.withdrawal_date);
}

class _VodafoneWithdrawalSummaryDetailState extends State<VodafoneWithdrawalSummaryDetail> {
  final withdrawal_date;
  _VodafoneWithdrawalSummaryDetailState({required this.withdrawal_date});
  late String username = "";
  final storage = GetStorage();
  bool hasToken = false;
  late String uToken = "";
  late List allVodafoneWithdraws = [];
  bool isLoading = true;
  late var items;
  late List amounts = [];
  ...
  fetchUserVodafoneWithdraws()async{
    ...
    if(response.statusCode ==200){
      ...
      allVodafoneWithdraws = json.decode(jsonData);
      amounts.assignAll(allVodafoneWithdraws);
    ...
    setState(() {
      isLoading = false;
      allVodafoneWithdraws = allVodafoneWithdraws;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Vodafone withdraws"),
      ),
      body: SafeArea(
          child:
          isLoading ? Column(
