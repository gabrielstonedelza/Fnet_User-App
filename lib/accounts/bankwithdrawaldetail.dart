...
class UserBankWithdrawalDetail extends StatefulWidget {
  final req_date;
  const UserBankWithdrawalDetail({Key? key,this.req_date}) : super(key: key);

  @override
  _UserBankWithdrawalDetailState createState() => _UserBankWithdrawalDetailState(req_date: this.req_date);
}

class _UserBankWithdrawalDetailState extends State<UserBankWithdrawalDetail> {
  final req_date;
  _UserBankWithdrawalDetailState({required this.req_date});
  ...
  fetchUserBankRequests()async{
    const url = "https://fnetghana.xyz/get_customer_bank_withdrawal_summary";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("$username's Bank Withdrawals"),
      ),
      body: SafeArea(
          child:
          isLoading ? Column(
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Text(items['date_requested'].toString().split("T").first,style: const TextStyle(fontSize: 14,fontWeight: FontWeight.bold,color: Colors.white),),
                                  ],
                                ),
                                Row(
                                  children: [
                                    ...
                                    Text(items['date_requested'].toString().split("T").last.split(".").first,style: const TextStyle(fontSize: 14,fontWeight: FontWeight.bold,color: Colors.white),),
                                  ],
                                ),
